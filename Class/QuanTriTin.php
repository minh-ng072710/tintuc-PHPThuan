<?php 
	...
	class QuanTriTin extends goc{
	   ...
	   $sql="SELECT idTL,TenTL,ThuTu,AnHien,TenTL_KhongDau,lang FROM theloai ORDER BY idTL ASC";
	   $kq = $this->db->query($sql) ;
	   ...
	   return $kq; 
		...
		function TheLoai_Them($TenTL, $TenTL_KD, $ThuTu,$AnHien,$lang){
			...
			if ($TenTL_KD=="") echo $TenTL_KD = $this->changeTitle($TenTL);
		...
		function DemTheLoai($idTL){
			$sql="select count(*) from theloai where idTL=$idTL;";
			$kq=$this->db->query($sql);
			$row=$kq->fetch_row();
			return $row[0];
...
function ListUser(){
			$sql="select users.idUser,users.HoTen,DiaChi,Email,Dienthoai,GioiTinh from users order by idUser ASC";
			$kq=$this->db->query($sql);
			if(!$kq) die($this->db->error);
			return($kq);
}
...
function Tin_Sua($TD, $TD_KD, $TT, $Ngay, $AnHien, $TNB, $urlHinh, $ND, $idTL, $idLT, $idTin,$tags,$lang){
  ...
  settype($idTL,"int");
  settype($idLT,"int");
  $sql="UPDATE tin SET TieuDe='$TD',TomTat='$TT' , 
     TieuDe_KhongDau='$TD_KD', Ngay='$Ngay', AnHien=$AnHien, 
     TinNoiBat=$TNB, urlHinh='$urlHinh', Content='$ND', idTL=$idTL, 
     idLT=$idLT,tags='$tags', lang='$lang' WHERE idTin=$idTin";
$kq= $this->db->query($sql) ;
  if(!$kq) die( $this-> db->error);
}
